...
*Generated by **DoogieHowser** | Last updated: 2026-02-19 — Chart Mechanisms update*
...
- 8 built-in trading strategies including 4 scalping strategies
...
└── src/
    ...
    │
    ├── strategies/
    │   ├── index.js            # Strategy registry and exports
    │   ├── cryptoMomentum.js   # Momentum trading (EMA crossovers, volume surge)
    │   ├── breakout.js         # Breakout above resistance with volume confirmation
    │   ├── vwapReclaim.js      # VWAP reclaim after dip
    │   ├── meanReversion.js    # Mean reversion at Bollinger Band extremes
    │   ├── rangeScalp.js       # Range-based scalping within support/resistance
    │   ├── microVwapScalp.js   # Micro VWAP scalping on 1-5m charts
    │   ├── momentumSpikeScalp.js # Momentum spike entry, reversal exit
    │   └── orderBookImbalanceScalp.js # Order book bid/ask imbalance scalping
    │
    └── components/
        ...
        ├── StrategyControls/   # Strategy toggles, bot controls, session scorecard
        │   ├── StrategyControls.jsx  # Main panel with regime grouping
        │   ├── StrategyCard.jsx      # Individual strategy toggle card
        │   ├── BotMasterControls.jsx # Start/stop all strategies
        │   └── ScalpSessionScorecard.jsx # Session stats (wins, losses, P&L, fees)
...
**Key actions:** `updateTicker`, `addCandle`, `setCandles`, `setIndicators`, `setRegime`, `addPosition`, `removePosition`, `updatePosition`, `setPortfolio`, `updatePaperPortfolio`, `updateWatchlist`, `toggleFavorite`, `setRiskSettings`, `toggleStrategy`, `emergencyStopAll`, `addToast`, `setIndicatorConfig`
...
| **StrategyControls** | Center | Strategy toggles grouped by regime, BotMasterControls (start/stop all), ScalpSessionScorecard (session stats), StrategyCard per strategy. Bearish mode disables all trading. |
...
**Bearish regime** disables all trading as a capital preservation mechanism.

### Strategies

Eight built-in strategies with a standardized interface:

```javascript
{
  meta: { name, description, regimes[], category, timeframes[], bestFor[], entryConditions[], exitConditions[] },
  requiredIndicators: string[],
  checkEntry(candles, indicators, orderBook, i) => { entry, direction, reason, confidence } | null,
  checkExit(position, candles, indicators, i) => { exit, reason } | null,
  riskOverrides: { stopLossMethod?, stopLossATRMult?, tp1R?, tp2R? }
}
```

| Strategy | Category | Regimes | Description |
|----------|----------|---------|-------------|
| **Crypto Momentum** | Trend | Bullish | EMA9 crosses EMA21, RSI > 50, volume surge confirmation |
| **Breakout** | Trend | Bullish | Close above 20-bar high with volume confirmation |
| **VWAP Reclaim** | Trend | Bullish | Price bounces off VWAP with trend alignment |
| **Mean Reversion** | Mean Reversion | Choppy | Price extremes (RSI < 30 or > 70) revert to mean |
| **Range Scalp** | Scalp | Choppy | Buy lows, sell highs within order book S/R levels |
| **Micro VWAP Scalp** | Scalp | Choppy | Scalp around VWAP on 1-5m charts |
| **Momentum Spike Scalp** | Scalp | Choppy | Enter on sudden momentum burst, exit on reversal |
| **Order Book Imbalance** | Scalp | Choppy | Large bid/ask imbalance signals direction |

**Adding a new strategy:** Create a file in `src/strategies/`, export an object matching the interface above, and add it to the registry in `src/strategies/index.js`. It will appear in the UI automatically.
...
### IndexedDB (`cerebro-crypto` database, v1)
...
| `STRATEGIES` | object | 8 strategy identifiers |
...
*Generated by DoogieHowser documentation agent on 2026-02-19*
